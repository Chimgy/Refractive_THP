...
export class AddSessionWallDurationBucketsToSdkTelemetryMetrics1786830000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "sdk_telemetry_metrics" ADD "sessionWallDurationBuckets" jsonb NOT NULL DEFAULT '[]'`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "sdk_telemetry_metrics" DROP COLUMN "sessionWallDurationBuckets"`,
    );
  }
}
