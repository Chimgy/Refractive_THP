...
export class TelemetryMetricsQueryService {
  ...
  async getSummary(projectId: string, days: number): Promise<TelemetrySummary> {
    ...
    return {
      ...
      lcpP50: weightedAverage(
        currentRows,
        (r) => r.lcpP50,
        (r) => r.lcpCount,
      ),
      lcpP75: weightedAverage(
        currentRows,
        (r) => r.lcpP75,
        (r) => r.lcpCount,
      ),
      lcpP99: weightedAverage(
        currentRows,
        (r) => r.lcpP99,
        (r) => r.lcpCount,
      ),
      ttfbP50: weightedAverage(
        currentRows,
        (r) => r.ttfbP50,
        (r) => r.ttfbCount,
      ),
      ttfbP75: weightedAverage(
        currentRows,
        (r) => r.ttfbP75,
        (r) => r.ttfbCount,
      ),
      ttfbP99: weightedAverage(
        currentRows,
        (r) => r.ttfbP99,
        (r) => r.ttfbCount,
      ),
      ...
      sessionWallDeltaMs:
        sessionWallAvgMs !== null && previousSessionWallAvgMs !== null
          ...
          : null,
      sessionWallP50: weightedAverage(
        currentRows,
        (r) => r.sessionWallP50,
        (r) => r.sessionWallCount,
      ),
      ...
      dnsColdP50: weightedAverage(
        currentRows,
        (r) => r.dnsColdP50,
        (r) => r.navColdCount,
      ),
      dnsColdP75: weightedAverage(
        currentRows,
        (r) => r.dnsColdP75,
        (r) => r.navColdCount,
      ),
      dnsColdP99: weightedAverage(
        currentRows,
        (r) => r.dnsColdP99,
        (r) => r.navColdCount,
      ),
      tcpColdP50: weightedAverage(
        currentRows,
        (r) => r.tcpColdP50,
        (r) => r.navColdCount,
      ),
      tcpColdP75: weightedAverage(
        currentRows,
        (r) => r.tcpColdP75,
        (r) => r.navColdCount,
      ),
      tcpColdP99: weightedAverage(
        currentRows,
        (r) => r.tcpColdP99,
        (r) => r.navColdCount,
      ),
      domContentLoadedColdP50: weightedAverage(
        currentRows,
        (r) => r.domContentLoadedColdP50,
        (r) => r.navColdCount,
      ),
      loadCompleteColdP50: weightedAverage(
        currentRows,
        (r) => r.loadCompleteColdP50,
        (r) => r.navColdCount,
      ),
      navColdCountries: mergeTopEntries(currentRows, 'navColdCountries', 20),
      navReusedCountries: mergeTopEntries(
        currentRows,
        'navReusedCountries',
        20,
      ),
      lcpColdP50: weightedAverage(
        currentRows,
        (r) => r.lcpColdP50,
        (r) => r.lcpColdCount,
      ),
      lcpColdP75: weightedAverage(
        currentRows,
        (r) => r.lcpColdP75,
        (r) => r.lcpColdCount,
      ),
      lcpColdP99: weightedAverage(
        currentRows,
        (r) => r.lcpColdP99,
        (r) => r.lcpColdCount,
      ),
      lcpCachedP50: weightedAverage(
        currentRows,
        (r) => r.lcpCachedP50,
        (r) => r.lcpCachedCount,
      ),
      lcpCachedP75: weightedAverage(
        currentRows,
        (r) => r.lcpCachedP75,
        (r) => r.lcpCachedCount,
      ),
      lcpCachedP99: weightedAverage(
        currentRows,
        (r) => r.lcpCachedP99,
        (r) => r.lcpCachedCount,
      ),
      ttfbHitP50: weightedAverage(
        currentRows,
        (r) => r.ttfbHitP50,
        (r) => r.ttfbHitCount,
      ),
      ttfbHitP75: weightedAverage(
        currentRows,
        (r) => r.ttfbHitP75,
        (r) => r.ttfbHitCount,
      ),
      ttfbHitP99: weightedAverage(
        currentRows,
        (r) => r.ttfbHitP99,
        (r) => r.ttfbHitCount,
      ),
      ttfbHitCount: sumBy(currentRows, (r) => r.ttfbHitCount),
      ttfbMissP50: weightedAverage(
        currentRows,
        (r) => r.ttfbMissP50,
        (r) => r.ttfbMissCount,
      ),
      ttfbMissP75: weightedAverage(
        currentRows,
        (r) => r.ttfbMissP75,
        (r) => r.ttfbMissCount,
      ),
      ttfbMissP99: weightedAverage(
        currentRows,
        (r) => r.ttfbMissP99,
        (r) => r.ttfbMissCount,
      ),
